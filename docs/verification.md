...
>
> **v7.0.0 note.** Three hardening changes (`create_pool` `sol_source` separation; swap-fee min-1 floor; `add_liquidity` SOL charge rounds UP). 1 new harness — `verify_proportional_ceil_rounds_up` — proves `calc_proportional_ceil ∈ {floor, floor+1}` (the pool-favoring rounding for the deposit's SOL charge), bringing the total to 25. `verify_swap_fee_threshold` was updated to the min-1 semantics (no nonzero swap is fee-free). (A read-side TWAP depth floor was considered and **rejected** — it would gate bad-debt liquidation on a thin/crashed pool; see audit.md.) All prior harnesses pass unchanged.
