...
import { getSwapQuote, buildSwapTransaction, TOKEN_2022_PROGRAM_ID } from 'deeppoolsdk'
...
export const SwapPanel = ({
  ...
  const handleSwap = async () => {
    ...
    setStatus('')
    try {
      const minOut = quote ? Math.floor((quote.amountOut * (10000 - slippageBps)) / 10000) : 1
  ...
  return (
    <div style={{ padding: '20px' }}>
      ...
      <div style={{ display: 'flex', gap: '6px', marginBottom: '12px' }}>
        {buy
          ? BUY_PRESETS.map((sol) => presetBtn(`${sol} SOL`, () => setAmount(sol.toString())))
      ...
      <div style={{ marginBottom: '12px' }}>
        <label
          style={{ fontSize: '12px', color: 'var(--muted)', marginBottom: '4px', display: 'block' }}
        >
          {buy ? 'SOL amount' : 'Token amount'}
          {!buy && tokenBalance > 0 && (
            <span style={{ float: 'right' }}>Balance: {(tokenBalance / 1e6).toFixed(2)}</span>
          )}
        </label>
        <input
          type="number"
      ...
      {quote && quote.amountOut > 0 && (
        ...
        >
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: '4px' }}>
            ...
            <span
              className="font-mono"
              style={{ color: quote.priceImpactPercent > 5 ? 'var(--danger)' : 'var(--muted)' }}
            >
